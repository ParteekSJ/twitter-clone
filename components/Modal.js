...
function Modal() {
  ...
  /**
   * When we comment, we set the `postId` in redux.
   * From there we retrieve the post from firebase using that postId and set it to local state.
   */
  useEffect(
    () =>
  ...
  const sendComment = async (e) => {
    ...
    // Successfully uploaded comment to database!
    dispatch(close());
    setComment("");
    router.push(`/${postId}`);
  };
        ...
        <div className="flex items-start justify-center min-h-[800px] sm:min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            <div className="inline-block align-bottom bg-black rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full">
              {/* CLOSE ICON */}
              <div className="flex items-center px-1.5 py-2 border-b border-b-gray-700">
                <div
              ...
              {/* SELECTED TWEET INFO  */}
              <div className="flexs px-4 pt-5 pb-2.5 sm:px-6">
                <div className="w-full">
                  <div className="text-[#6e767d] flex gap-x-3 relative">
                    {/* GRAY PIPE */}
                    <span className="w-0.5 h-full z-[-1] absolute left-5 top-11 bg-gray-600" />
                    {/* SELECTED TWEET'S USER IMAGE */}
                    <img
